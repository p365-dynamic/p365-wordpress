...
function wplc_tirggers_enqueue_user_styles_scritps($trigger_id){
	...
	wp_register_script("wplc_trigger_js",  plugins_url('/js/wplc_user_triggers.js', __FILE__), array('jquery'), $wplc_pro_version);
	/*Localize vars here*/
	$trigger_id = intval($trigger_id);
	...
	if($trigger_data !== false && is_array($trigger_data)){
		...
		if(intval($trigger_data->show_content) == 1){
			wp_enqueue_style("wplc_trigger_styles",  plugins_url('/css/wplc_trigger_styles.css', __FILE__), array(), $wplc_pro_version);
		}
