...
class WPAM_List_Affiliates_Table extends WPAM_List_Table {
    ...
    function get_columns() {
        $columns = array(
            ...
            'email' => __('Email ID', 'affiliates-manager'),
            'companyName' => __('Company Name', 'affiliates-manager'),
    ...
    function get_bulk_actions() {
        $actions = array(
            'deactivate' => __('Deactivate', 'affiliates-manager'),
            'activate' => __('Activate', 'affiliates-manager')
        );
        return $actions;
    }
    ...
    function process_bulk_action() {
        ...
        if ('deactivate' === $this->current_action()) {
            ...
            foreach ($records_to_delete as $row) {
                ...
                $updatedb = "UPDATE $record_table_name SET status = 'inactive' WHERE affiliateId='$row'";
                $results = $wpdb->query($updatedb);
            }
            echo '<div id="message" class="updated fade"><p>' . __('Selected Affiliates Deactivated successfully!', 'affiliates-manager') . '</p></div>';
        }
        ...
        if ('activate' === $this->current_action()) {
            ...
            foreach ($records_to_delete as $row) {
                ...
                // }
                // $updatedb = "DELETE FROM $record_table_name WHERE affiliateId='$row'";
                $updatedb = "UPDATE $record_table_name SET status = 'active' WHERE affiliateId='$row'";
                $results = $wpdb->query($updatedb);
            }
            echo '<div id="message" class="updated fade"><p>' . __('Selected Affiliates Activated successfully!', 'affiliates-manager') . '</p></div>';
        }
    }
