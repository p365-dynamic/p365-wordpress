...
if (!get_user_meta($user->ID, 'campaignsid', true)){
	add_user_meta( $user->ID, 'campaignsid', '1', false );
}
if (isset($_POST['submit'])) {
//echo "<div id='message' class='updated'><p>Hello  Hello</p></div>";
	$campaigns = $_POST['campaigns'];
	$campaignLinks = get_user_meta($user->ID, 'campaignsid', true);
	$campaignLinks = explode(',',$campaignLinks);
	//print_r($campaigns);
	//print_r($campaignLinks);
	if(in_array($campaigns, $campaignLinks))
	{
		echo "<div id='message' class='updated'><p>Campaign Already assigned to user</p></div>";
	}
	else{
		// print_r($campaigns);
		
		$camp = (string)$campaigns;
		...
		if(!in_array($camp,$existing_columns)){
			$mydb->query("ALTER TABLE aff_camp_visits ADD `{$camp}` INT(11) DEFAULT NULL");
		}
		$existing_columns_sales = $mydb->get_col("DESC aff_sales", 0);
		if(!in_array($camp,$existing_columns_sales)){
			$mydb->query("ALTER TABLE aff_sales ADD `{$camp}` INT(11) DEFAULT NULL");
		}
		$existing_columns_premium = $mydb->get_col("DESC aff_premium", 0);
		if(!in_array($camp,$existing_columns_premium)){
			$mydb->query("ALTER TABLE aff_premium ADD `{$camp}` double DEFAULT NULL");
		}
		// $existing_columns1 = $mydb->get_col("DESC aff_premium", 0);
		// if(!in_array($campaigns,$existing_columns1)){
		// 	$mydb->query("ALTER TABLE aff_premium ADD $campaigns double DEFAULT NULL");
		// 	print_r('execute for premium');
		// }

		// $existing_columns2 = $mydb->get_col("DESC aff_sales", 0);
		// if(!in_array($campaigns,$existing_columns2)){
		// 	$mydb->query("ALTER TABLE aff_sales ADD $campaigns INT(11) DEFAULT NULL");
		// 	print_r('execute for sales');
		// }
		// die('sssssss');
		array_push($campaignLinks,$campaigns);
	...
	// print_r($updresponse);
	// die();
		update_user_meta($user->ID, 'campaignsid', $campaignLinks, false );
...
<script type="text/javascript">
	...
	jQuery(document).ready(function() {
	    ...
	    jQuery('.delete_class').click(function(){
		    var userInfo = "<?php echo $user->ID ?>";
			var affid = "<?php echo $affiliateID?>";
            var tr = jQuery(this).closest('tr'),
                del_id = jQuery(this).attr('id');
			console.log("test var",del_id);
			if(del_id == "1"){
				alert("Default Campaign Cannot be Removed");
			}else{
			jQuery.ajax({
				 type: "POST",
				 url: ajaxurl,
                 ...
                 success:function(result){
					tr.html("<div id='message' class='updated'><p>Campaign Removed successfully!</p></div>");
...
*/
		...
		jQuery("#_ddBountyType").change(function() {
			...
			else if (type == 'cost_per_visit'){
				jQuery('#_lblBountyAmount').html(currencyL10n.fixedLabel);
			}
			else if (type == 'cost_per_lead'){
				jQuery('#_lblBountyAmount').html(currencyL10n.fixedLabel);
			}
		})
	});
		...
		<tr>
			...
			<td>
				<select id="_ddBountyType" name="ddBountyType">
					...
					<option value="cost_per_visit"><?php _e( 'Cost Per Visit', 'affiliates-manager' ) ?></option>
					<option value="cost_per_lead"><?php _e( 'Cost Per Lead', 'affiliates-manager' ) ?></option>
				</select>
			</td>
		</tr>
		<tr>
...
<div id="tabs">
    <ul>
        ...
        <li><a href="#fragment-2"><span><?php _e( 'Edit Earnings', 'affiliates-manager' ) ?></span></a></li>
		<li><a href="#fragment-3"><span><?php _e( 'Contact details', 'affiliates-manager' ) ?></span></a></li>
        <li><a href="#fragment-4"><span><?php _e( 'Add Campaign', 'affiliates-manager' ) ?></span></a></li>
...
<?php } ?>
		...
		<table class="widefat">
			...
			<tbody>
				<tr><td width="200"><?php _e( 'Date Registered', 'affiliates-manager' ) ?></td><td><?php echo date("d/M/Y H:i:s", $model->dateCreated)?></td></tr>
				...
				<tr>
					<td width="200">
						<?php _e( 'Commission Standing', 'affiliates-manager' ) ?>
					</td>
					<td>
		...
		<table class="widefat">
			...
			<tbody>
		<tr><td width="200"><?php _e( 'Commission Type', 'affiliates-manager' ) ?></td><td><?php echo $model->getBountyType() ?></td></tr>
				<tr><td><?php _e( 'Commission Amount', 'affiliates-manager' ) ?></td><td><?php echo $model->bountyAmount?></td></tr>
			<?php if ($model->isConfirmed() || $model->isActive()) { ?>
			<tr><td><?php _e( 'Payment Method', 'affiliates-manager' ) ?></td><td><?php echo "Cheque"/*$model->getPaymentMethod()*/ ?></td></tr>
	...
	<div id="fragment-4">
		...
		<?php 
						...
						//print_r(get_user_meta($user->ID));
							//print_r(get_user_meta($user->ID));
							//update_user_meta($user->ID, 'campaignsid', '1,4', false );
							//print_r(get_user_meta($user->ID));
							$campaignLinks = get_user_meta($user->ID, 'campaignsid', true);
							$campaignLinks = explode(',',$campaignLinks);
							//print_r($campaignLinks);
		...
		<table class="widefat">
			<thead>
			<tr>
				<th><?php _e( 'Campaign', 'affiliates-manager' ) ?></th>
			...
			<?php 
				...
				foreach ( $rows as $creative ) {
				// $linkBuilder = new WPAM_Tracking_TrackingLinkBuilder($model, $creative);
				// $link = $linkBuilder->getImpressionHtmlSnippet();
				//print_r($link);
				?>
			<?php if(in_array($creative->id, $campaignLinks)){ ?>
			<tr class="creative-<?php echo $creative->status?>" class="<?php echo $creative->creativeId?>">
				<td><?php echo $creative->name?></td>
				<!-- <td><?php echo $creative->type?></td> -->
				<td><input type="text" size="50" value='<?php echo get_home_url()."/?campaign_id=".$creative->id."&wpam_id=".$affiliateID?>' /></td>
				<td><input type="button" id ="<?php echo $creative->id?>" value = "-" class="button-secondary delete_class"></td>
			</tr>
			<?php }?>
			<?php } ?>
			</tbody>
		</table>
		<div><p>Add Campaign</p>
		<form method="post">
		<!-- <select name="campaigns">
		<?php foreach ( $this->viewData['creatives'] as $creative ) { ?>
  		<option value="<?php echo $creative->creativeId ?>"><?php echo $creative->name ?></option>
		<?php }?>
		</select> -->
		<select name="campaigns">
		<?php
print_r("data coming from campaigns");
	...
	<?php endforeach; ?>
		...
		<input type='hidden' name='input_name' value="<?php echo htmlentities(serialize($campaignLinks)); ?>" />
		<input type="submit" name="submit" class="button-primary" value="ADD Campaign for user">
		</form>
		</div>
  </div>
</div>
