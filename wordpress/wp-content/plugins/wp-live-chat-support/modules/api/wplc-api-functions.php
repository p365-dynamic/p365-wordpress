				...
				if(isset($request['chat_id'])){
					if(isset($request['agent_id'])){
						...
						do_action("wplc_hook_update_agent_id",$request['cid'],intval($request['agent_id']));
							$result_chat_session = $wpdb->get_row("SELECT * FROM $wplc_tblname_chats WHERE `rel` = '$rel_upd' ", ARRAY_A);
							if($result_chat_session['leadmsgid']==''){
								$LeadMSGID = "userinit";
		
									$upd = $wpdb->update( 
										$wplc_tblname_chats, 
										array( 
											'leadmsgid' => $LeadMSGID	// string
										), 
										array( 'rel' =>  $rel_upd),
										array(
											'%s'
								...
								$LeadMSGID = "Lead Present Already";
								// print_r('lead present');
							
							}							  
							

							$return_array['response'] = $LeadMSGID;
						...
						if(isset($request['relay_action'])){
								...
								if($action == "wplc_user_send_msg"){
									...
									$return_array['data'] = array("chat_id" => intval($request['chat_id']),
										...
										// $cookie_name = 'Nolead';
										$cookie_lead = 'messageId';
										if(!isset($_COOKIE[$cookie_lead])) {
											// get details from db
											$result_chat_session = $wpdb->get_row("SELECT * FROM $wplc_tblname_chats WHERE `rel` = '$chat_id' ", ARRAY_A);
											// print_r($result_chat_session);
											if($result_chat_session['leadmsgid'] == 'agentinit' || $result_chat_session['leadmsgid'] == 'userinit'){
												$header = array(
													...
													"body"=> array(
														...
														"lastName" => "From Chat",
														 "requestSource" => "chat",
														  "campaign_id" => "c8d6de36-7055-df32-b88a-5c33409dce9a",
														  "leadSource" => array(
															"utm_campaign" => "",
															"utm_source" => ""
														  )
												...
												// $updresponse = json_decode($updresponse);
												$resp1 = json_decode($updresponse['body']);
												$resp1 = json_decode($resp1);
												print_r($resp1->data->messageId);
												$LeadMSGID = $resp1->data->messageId;
													...
													// $leadid = $result->id;
													setcookie('messageId', $LeadMSGID, time() + 84000, "/", COOKIE_DOMAIN);
											}
											else{
											setcookie('messageId', $result_chat_session['leadmsgid'], time() + 84000, "/", COOKIE_DOMAIN);
											// remove nolead cookie
											// setcookie('Nolead', '', time() - 3600, "/", COOKIE_DOMAIN);
											} 	
										}
	...
	if(isset($request)){
		...
		if( isset( $request['server_token'] ) ){
			...
			if( isset( $request['wplc_name'] ) && isset( $request['wplc_email'] ) && isset( $request['session'] ) ){
				...
				$cookie_name = 'messageId';
				if(!isset($_COOKIE[$cookie_name])) {
					// setcookie('Nolead', 'true', time() + 84000, "/", COOKIE_DOMAIN);
					// print_r('set_no_lead');
					$wpdb->insert( 
						$wplc_tblname_chats, 
						array( 
				...
				$LeadMSGID = $_COOKIE[$cookie_name];
					// insert lead msg id if available along with chat
		        $wpdb->insert( 
		            $wplc_tblname_chats, 
		            array( 
		                'status' => 2, 
		...
		// 		$username = "admin";
		// 		$password = "bf4e8a0bb03dbb483f575fbc43711c0d";
		...
		// 				'cookies' => array()
		// 	);
		// 	$response_id = wp_remote_post( 'http://uatcrm.policies365.com/service/v4_1/rest.php', $args);
		// 	$resp1 = json_decode($response_id['body']);
		// 	$sessionId = $resp1->id;
		// 	print_r($sessionId);
		...
		// 	$name_value_list = json_encode($name_value_list);
		...
		// 	 $set_entry_params_arr = json_encode($set_entry_params);

		// 	 $parameters = array(
		// 		"application_name" => "front-end",
		// 		"name_value_list" => $set_entry_params_arr,
		// 		);
		// 	$jsonEncodedData = json_encode($parameters);
		...
		// 			'cookies' => array()
		// );
		// $response_lead = wp_remote_post( 'http://uatcrm.policies365.com/service/v4_1/rest.php', $args);
		// print_r($response_lead);	
		// die();
