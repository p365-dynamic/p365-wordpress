...
class WPAM_Util_UserHandler {
    ...
    public function AutoapproveAffiliate($affiliate, $bountyType = '', $bountyAmount = '') {
        ...
        $mailer = new WPAM_Util_EmailHandler();
        $db = new WPAM_Data_DataAccess();
        $affname = $affiliate->firstName." ".$affiliate->lastName;
        $affemail = $affiliate->email;
        $affdesc = $affiliate->companyName;
        // print_r($affiliate);
...
$header = array(
   ...
   "transactionName" => "createAffiliate"),
   "body"=> array("action"=>"CREATE",
   "source"=> "Web", 
   ...
   "emailId" => $affemail,
   "campaignId"=> "", 
   "affilateURL"=> "http://p365dev.infintus.com/?wpam_id=".$affiliateID, 
   "affilateUser"=> false,
   "redirectToAffilateScreen"=> false 
   )
);
...
$createresponse = wp_remote_post( 'http://192.168.0.10:8181/cxf/authrestservices/integrate/invoke', $args);
