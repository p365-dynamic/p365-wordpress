...
class WPAM_Pages_AffiliatesHome extends WPAM_Pages_PublicPage
{
			...
			if ($db->getAffiliateRepository()->isUserAffiliate($currentUser->ID))
			{
				...
				if ($affiliate->isApproved() || $affiliate->isActive())
				{
					...
					$response->viewData['navigation'] = array(
						...
						array( __( 'Campaigns', 'affiliates-manager' ), $this->getLink(array('sub' => 'creatives'))),
	...
	protected function doCreatives($request)
	{
		...
		if ( isset( $request['action'] ) && $request['action'] == 'detail')
		{
			...
			if ($creative === NULL)
				wp_die( __( 'Invalid campaign.', 'affiliates-manager' ) );
			if ($affiliate === NULL)
				wp_die( __( 'Invalid affiliate', 'affiliates-manager' ) );
			if (!$creative->isActive())
				wp_die( __( 'Inactive campaign.', 'affiliates-manager' ) );
	...
	protected function doOverviewHome($request, $affiliate)
	{
		...
		$response->viewData['accountCredits'] = $accountSummary->credits;
        $response->viewData['accountDebits'] = $accountSummary->debits;
	...
	protected function doContactInfo( $request, $affiliate ) {
		...
		if (isset($request['action']) && $request['action'] == 'saveInfo')
		{
			...
			if ($vr->getIsValid()) {
				...
				$affiliateID = $affiliate->affiliateId;
				$nameaff = $_POST['_firstName']." ".$_POST['_lastName'];
				$description = $_POST['_companyName'];
				// print_r($affiliateID);
				// print_r($nameaff);
				// print_r($description);
				// die();
				...
				//$updresp1 = json_decode($response['body']);
				//print_r($response);
				//$updresp1 = json_decode($updresp1);
				//print_r($updresp1);
