jQuery(function() {
    ...
    jQuery("body").on("click", "#wplc_add_agent", function(e) {
        ...
        var regex_usern = /[a-z0-9\S*$\_$]{8,}$/;

        if (!username || !email || !password) {
            var wplchtml = "<p class = 'error_msg' style='color:red;'>All fields are required</p>";
            jQuery(wplchtml).insertBefore("#wplc_add_agent");
            jQuery(".error_msg").fadeOut(3000);
        } else if (!regex_email.test(email)) {
            var wplchtml = "<p class = 'error_msg' style='color:red;'>Invalid Email Entered</p>";
            jQuery(wplchtml).insertBefore("#wplc_add_agent");
            jQuery(".error_msg").fadeOut(3000);
        } else if (!regex_usern.test(username)) {
            var wplchtml = "<p class = 'error_msg' style='color:red;'>Invalid Username Entered. Username must be minimum 8 charactors</p>";
            jQuery(wplchtml).insertBefore("#wplc_add_agent");
            jQuery(".error_msg").fadeOut(3000);
        } else {
            ...
            jQuery.post(ajaxurl, data, function(response) {
                if (response) {
                    ...
                    var wplchtml = "<p class = 'error_msg'>" + response + "</p>";
                    jQuery(wplchtml).insertBefore("#wplc_add_agent");
                    jQuery(".error_msg").fadeOut(3000);
